...
def send_email_window(file_name):
    def send():
        ...
        sender_email = os.getenv("CONVOLOGIX_SMTP_FROM_EMAIL") or os.getenv("CONVOLOGIX_SMTP_USERNAME")
        sender_password = os.getenv("CONVOLOGIX_SMTP_PASSWORD")
        if not sender_email or not sender_password:
            messagebox.showerror("Error", "SMTP email and password are not configured in environment variables.")
            return
